...
import com.ikemba.inventrar.core.presentation.CustomRed
...
import com.ikemba.inventrar.heldOrder.presentation.component.ConfirmVoid
...
import com.ikemba.inventrar.transactionHistory.presentation.formatDateFilter
...
fun HeldOrderScreen(viewModel: HeldOrderViewModel = koinViewModel(),dashboardViewModel: DashboardViewModel) {
    ...
    LaunchedEffect(true){
        viewModel.getHeldOrders(paginationRequest.value)
    }
    Scaffold() {
        Box(modifier = Modifier.fillMaxSize().background(CustomGrey)) {
            ...
            Column(modifier = Modifier.fillMaxSize().padding(24.dp).background(Color.Transparent)) {
                Row(
                    ...
                    Column() {
                        CustomText("Paused Orders", size = 24.sp, color = Color.Black)
                        CustomText(
                            "Here is a list of all orders on hold",
                            size = 14.sp,
                            color = Color.Gray
                        )
                    }
                    Row(
                        ...
                        HeldOrderDropdownMenuSample(paginationRequest = paginationRequest)
                        ...
                        AnimatedVisibility(
                            ...
                            HeldOrderDateRangePickerModal(onDismiss = {
                ...
                Column(modifier = Modifier.fillMaxSize().background(Color.White).padding(20.dp)) {
                    ...
                    Column(modifier = Modifier.fillMaxHeight(0.9f)) {
                        ...
                        Row(
                            ...
                            Row(
                                ...
                                CustomText("show: ")
                                HeldOrderNumberPicker(paginationRequest= paginationRequest)
                                CustomText("Entries: ")
                            }
                            Row(modifier = Modifier.width(350.dp)) {
                                HeldOrderSearchBar(paginationRequest = paginationRequest)
                            }
                    ...
                    HeldOrderPaginationControls(viewModel, paginationRequest)
                }
            }
            if (state.value.heldOrders.orders!!.isEmpty()) {
                Column(
...
@Composable
fun HeldOrderSearchBar(viewModel: HeldOrderViewModel = koinViewModel(), paginationRequest: MutableState<PaginationRequest>) {
            ...
            BasicTextField(
                ...
                onValueChange = {
                    ...
                    debounceJob = coroutineScope.launch {
                        delay(1500)
                        viewModel.searchHeldOrder(it, paginationRequest)
                    }
...
fun HeldOrderTransactionTable(viewModel: HeldOrderViewModel, dashboardViewModel: DashboardViewModel, paginationRequest: MutableState<PaginationRequest>) {
    ...
    LazyColumn(modifier = Modifier.fillMaxWidth().heightIn(max = 3000.dp, min = 0.dp)) {
        ...
        items(state.value.heldOrders.orders!!){ transaction ->
            val index = state.value.heldOrders.orders!!.indexOf(transaction)
            Row(modifier = Modifier.fillMaxWidth(0.8f).padding(horizontal = 19.dp, vertical = 8.dp), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(10.dp)) {
                ...
                HeldOrderTableCell(((state.value.heldOrders.limit * (state.value.heldOrders.page-1) +index + 1).toString()).toString(), modifier = Modifier.width(200.dp))
                ...
                Row( modifier = Modifier.width(300.dp), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.Center){
                    Button({
                        viewModel.getVoidedOrder(transaction.orderId, dashboardViewModel)
                    }, colors = ButtonDefaults.buttonColors(containerColor = PrimaryColor)){
                        Row(verticalAlignment = Alignment.CenterVertically){
                            Icon(
                                imageVector = Icons.Default.KeyboardArrowRight, contentDescription = ""
                            )
                            CustomText("Proceed", color = Color.White)
                        }
                    }
                }
                Row( modifier = Modifier.width(300.dp), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.Center){
                    Button({
                        viewModel.setVoidOrderReferenceAndShowConfirmVoidDialog(transaction.orderId)
                    }, colors = ButtonDefaults.buttonColors(containerColor = CustomRed)){
                        Row(verticalAlignment = Alignment.CenterVertically){
                            Icon(
                                imageVector = Icons.Default.Close, contentDescription = ""
                            )
                            CustomText("Void Order", color = Color.White)
                        }
                    }
                }
            ...
            ConfirmVoid(paginationRequest = paginationRequest.value )
    }
        VerticalScrollbar(
...
@Composable
fun HeldOrderPaginationControls(viewModel: HeldOrderViewModel, paginationRequest: MutableState<PaginationRequest>) {
    var debounceJob by remember { mutableStateOf<Job?>(null) }
    val coroutineScope = rememberCoroutineScope()
    val state = viewModel.state.collectAsStateWithLifecycle()
    ...
    Row(
        ...
        Spacer(modifier = Modifier.width(spacerWidth))
        PaginationButton(onClick = { /* Current Page */ }, text = state.value.page.toString(), enabled = false)
        Spacer(modifier = Modifier.width(spacerWidth))
        PaginationButton(onClick = {
            debounceJob?.cancel()
            viewModel.hideAllDialogs()
...
@Composable
fun HeldOrderDropdownMenuSample(viewModel: HeldOrderViewModel = koinViewModel(), paginationRequest: MutableState<PaginationRequest>) {
...
fun HeldOrderDateRangePickerModal(
    ...
    onDismiss: () -> Unit, viewModel: HeldOrderViewModel = koinViewModel(), paginationRequest: MutableState<PaginationRequest>
) {
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun HeldOrderNumberPicker(viewModel: HeldOrderViewModel = koinViewModel(), paginationRequest: MutableState<PaginationRequest>) {
